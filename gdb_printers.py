...
class MessagePrinter(gdb.ValuePrinter):
  """Allows a message to be on the rhs of =.  Not very useful I think as
     ArrayPrinter will do the same with a python string.
  """
  def __init__(self, val : gdb.Value, string) -> None:
    self.val = val
    self.string = string
...
class ArrayPrinter(gdb.ValuePrinter):
  def __init__(self, val : gdb.Value) -> None:
    ...
    if t.target().unqualified() in CHAR_TYPES:
      self.summary = self.val.lazy_string(length=n)  # GDB handles quotes/escaping
      if str(self.summary) == '<error: Cannot access memory at address 0x0>':
        # This is a python string so don't quote it
        self.summary = self.val.string(length=n)
        self.python_string = True
    elif n == 0:
      self.summary = ""
    else:
      self.summary = f"[...] length = {n}"
...
class RawPrinter(gdb.ValuePrinter):
  ...
  def to_string(self):
    if self.printer is not None and 'summary' in self.printer:
      summary = self.printer['summary'](self.val)
    else:
      summary = ""
    return summary
...
class ViewPrinter(gdb.ValuePrinter):
  ...
  def __init__(self, val : gdb.Value, view):
    self.val = val
    self.view = view
...
def _lookup_type(val):
  ...
  if '(****)' in type_str:
    ...
    actual_val_type_str = str(actual_val.type)
